...
Querydsl을 사용하기 위해 프로젝트 설정부터 차근차근 달려봅시다!
...
`Querydsl`을 사용하면서 어떻게 보면 가장 불편한 점이 `gradle` 설정하는 부분이라고 할 수 있는데요, `lombok`도 예전에는 꽤 불편하게 설정했던 것으로 기억하는데 지금은 아주 단순해진 거 보면 이
부분도 나아지지 않을까 기대해봅니다. 😁
...
저는 개인적으로 Querydsl의 기능은 매우 좋다고 생각하지만 여기서 사용하는 컨벤션 만큼은 정말 별로라고 생각합니다.
